...
enum GameAliasResolver: Sendable {
    ...
    nonisolated static func resolve(query: String) -> String {
        ...
        // 1. Exakt matchning
        if let replacement = exactAliases[normalized] {
            return replacement
        }

        // 2. Prefix-matchning för förkortningar
        for (alias, replacement) in exactAliases {
            if normalized.hasPrefix(alias + " ") {
                let remainder = trimmed.dropFirst(alias.count).trimmingCharacters(in: .whitespaces)
